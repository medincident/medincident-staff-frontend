...
function EventItem({ evt, typeMap, catMap }: { evt: IncidentEvent, typeMap: Record<string, string>, catMap: Record<string, string> }) {
  ...
  return (
    <Link
      href={`/events/${evt.id}`}
      className={`
        group flex items-center justify-between p-4 bg-card border rounded-xl transition-all cursor-pointer border-border'}
      `}
    >
...
function RequestItem({ req }: { req: ServiceRequest }) {
  ...
  return (
    <Link
      ...
      className="group flex items-center justify-between p-4 bg-card border border-border rounded-xl transition-all cursor-pointer hover:border-primary/30"
    >
