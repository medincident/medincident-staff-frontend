...
import { RequestChatContainer } from "@/components/requests/request-chat-container";
...
export default function RequestDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleStatusChange = async (newStatus: RequestStatus) => {
    ...
    try {
        ...
        toast.success("Статус обновлен", `Заявка переведена в статус "${STATUS_MAP[newStatus]}"`);
    } catch (error) {
        console.error(error);
           ...
           <Card className="gap-3 bg-card border">
                <CardHeader>
                    <div className="flex items-center justify-between mb-1">
                        ...
                        <Badge variant="outline" className={`shadow-none border-0 ${getBadgeColor(request.priority)}`}>
  ...
  if (!request) return <div className="p-8 text-center">Заявка не найдена</div>;
    ...
    <div className="flex flex-col gap-4 h-[calc(100vh-6rem)] md:h-auto pb-4 md:pb-20">
      ...
      <div className="md:hidden flex-1 flex flex-col min-h-0">
        <Tabs defaultValue="details" className="flex-1 flex flex-col min-h-0">
            ...
            <TabsContent value="chat" className="flex-1 mt-0 h-full overflow-hidden"><RequestChatContainer requestId={id} className="h-full" /></TabsContent>
        </Tabs>
      </div>

      <div className="hidden md:grid grid-cols-3 gap-6 items-start">
        <div className="col-span-2 space-y-6"><DetailsSection /></div>
        <div className="col-span-1 sticky top-24 h-[600px]"><RequestChatContainer requestId={id} /></div>
      </div>
    </div>
  );
}