...
export function UsersView() {  
  // --- STATE ---
  const organizationId = "YOUR_ORG_ID"; // ID текущей организации
  ...
  const loadData = async () => {
    try {
      ...
      // Загружаем список пользователей (включая деактивированных)
      const usersListRes = await UsersService.listUsers(true, search || undefined, undefined, 50);
      
      // Чтобы получить данные о трудоустройстве (employment) и ФИО, запрашиваем детали каждого пользователя
      const fullUsers = await Promise.all(
        usersListRes.items.map(u => UsersService.getUserById(u.id))
      );
      setUsers(fullUsers);

      // Загружаем структуру клиник и отделений для селектов
      if (clinics.length === 0) {
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      // 3. Обновляем место работы, если выбрано отделение
        ...
        <DialogContent>
            ...
            <div className="grid gap-4 py-4 max-h-[60vh] overflow-y-auto pr-2">
                <div className="grid gap-2">
                    ...
                    <Select value={formData.isActive ? "active" : "deactivated"} onValueChange={(v) => setFormData({...formData, isActive: v === "active"})}>
                ...
                <div className="grid gap-2 border-t pt-4">
                    ...
                    <Input value={formData.position} onChange={(e) => setFormData({...formData, position: e.target.value})} placeholder="Например: Медбрат" />
                </div>
                ...
                {!formData.isAdmin && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
                        <div className="grid gap-2">
                            <Label>Клиника</Label>
                            <Select value={formData.clinicId} onValueChange={(v) => setFormData({...formData, clinicId: v, departmentId: ""})}>
