// Метод Holt (двойное экспоненциальное сглаживание) для прогноза временных
// рядов с линейным трендом.
//
// Идея простым языком:
//   level — "где находимся сейчас" (усреднённый недавний уровень).
//   trend — "куда движемся" (насколько точка меняется от шага к шагу).
//   alpha — насколько быстро реагируем на новое значение (0..1).
//   beta — насколько быстро подстраиваем тренд.
//
// Для доверительного интервала используем стандартное отклонение остатков
// (разницы между предсказанием и фактом в истории) и расширяем зону
// неопределённости по горизонту прогноза — чем дальше заглядываем, тем шире.
