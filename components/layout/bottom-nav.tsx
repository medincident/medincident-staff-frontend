...
export function BottomNav() {
  ...
  return (
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 h-16 pb-safe bg-card border-t border-border shadow-[0_-1px_3px_rgba(0,0,0,0.05)] transition-colors duration-300">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive 
                  // ИСПРАВЛЕНО: text-primary (зеленый)
                  ? "text-primary" 
                  // ИСПРАВЛЕНО: text-muted-foreground hover:text-foreground
                  : "text-muted-foreground hover:text-foreground"
              )}
            >
              <div className={cn(
                "relative flex items-center justify-center p-1 rounded-xl transition-colors",
                // ИСПРАВЛЕНО: bg-primary/10 (полупрозрачный зеленый фон)
